...
impl DocumentConverter {
    ...
    pub fn convert(&self, source: SourceDocument) -> Result<ConversionResult, ConversionError> {
        ...
        let mut document = match source.format {
            ...
            InputFormat::Pdf => docling_crab_pdf::convert(&source.bytes, None, &source.name)
                .map_err(|e| ConversionError::Parse(e.to_string()))?,
            InputFormat::Image => docling_crab_pdf::convert_image(&source.bytes, &source.name)
                .map_err(|e| ConversionError::Parse(e.to_string()))?,
            InputFormat::MetsGbs => {
                docling_crab_pdf::convert_mets_gbs(&source.bytes, &source.name)
                    .map_err(|e| ConversionError::Parse(e.to_string()))?
            }
